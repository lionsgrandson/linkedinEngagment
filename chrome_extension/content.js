;(() => {
  ...
  function status(text) {
    panel()
    document.getElementById('cc-status').textContent = text
  }
  ...
  async function handleProfileConnection() {
    ...
    await delay()
    const messageButton = [...document.querySelectorAll('button,a')].find(
      (e) => e.offsetParent !== null && e.textContent.trim() === 'Message',
    )
    if (messageButton) {
      if (task.mode !== 'acceptedCheck') {
        status('Already connected — no request sent')
        return
      }
      const response = await api('/draft-message', 'POST', {
        stage: 'accepted',
        context: (document.querySelector('main')?.innerText || '').slice(
          0,
          5000,
        ),
      })
      if (!response?.ok || !response.data.allowed || !response.data.message)
        return status(
          `Opener skipped — ${response?.data?.reason || 'draft failed'}`,
        )
      messageButton.click()
      await delay()
      const editor = [
      ...
      editor.focus()
      document.execCommand('insertText', false, response.data.message)
      if (!(await countdown('Non-pitch opener'))) return
      const sendMessage = [...document.querySelectorAll('button')].find(
        (e) => e.offsetParent !== null && /^Send$/i.test(e.textContent.trim()),
      )
      if (!sendMessage || sendMessage.disabled)
        return api('/result', 'POST', {
          ...
          kind: 'message',
          url: task.url,
          reason: 'message Send button unavailable',
        })
      sendMessage.click()
      status('Non-pitch opener submitted')
      await api('/result', 'POST', {
        ok: true,
        kind: 'message',
        url: task.url,
        reason: 'accepted-connection opener submitted',
      })
    ...
    let connect = [...document.querySelectorAll('button')].find(
      (e) => e.offsetParent !== null && e.textContent.trim() === 'Connect',
    )
    if (!connect) {
      const more = [...document.querySelectorAll('button')].find(
        (e) => e.offsetParent !== null && e.textContent.trim() === 'More',
      )
      more?.click()
      await sleep(500)
      connect = [
    ...
    if (!connect)
      return api('/result', 'POST', {
        ok: false,
        kind: 'connection',
        reason: 'Connect control not found',
      })
    connect.click()
    await sleep(500)
    const addNote = [...document.querySelectorAll('button')].find(
      (e) => e.offsetParent !== null && e.textContent.trim() === 'Add a note',
    )
    addNote?.click()
    await sleep(300)
    ...
    if (input) {
      input.value = response.data.message.slice(0, 300)
    ...
    const send = [...document.querySelectorAll('button')].find(
      (e) =>
        e.offsetParent !== null &&
        /^(Send|Send invitation)$/.test(e.textContent.trim()),
    )
    if (!send || send.disabled)
      return api('/result', 'POST', {
        ok: false,
        kind: 'connection',
        reason: 'Send invitation unavailable',
      })
    send.click()
    status('Connection request submitted')
    await api('/result', 'POST', {
      ok: true,
      kind: 'connection',
      url: task.url,
      reason: 'connection request submitted',
    })
    setTimeout(() => window.close(), 1200)
  }
  async function maybeCheckAcceptedConnection() {
    ...
    localStorage.setItem(
      'ccPendingConnection',
      JSON.stringify({ url, mode: 'acceptedCheck', created: Date.now() }),
    )
  ...
  async function maybeDailyPost(samples) {
    ...
    const response = await api('/daily-post', 'POST', {
      samples: samples.map((item) => item.text),
    })
    const draft = response?.data?.draft
    if (!response?.ok || !response.data.allowed || !draft) return
    const start = [...document.querySelectorAll('a,button')].find(
      (element) => element.textContent.trim() === 'Start a post',
    )
    if (!start)
      return api('/result', 'POST', {
        ok: false,
        kind: 'post',
        reason: 'Start a post control not found',
      })
    start.click()
    await delay()
    const editor = [
    ...
    const submit = [...document.querySelectorAll('button')].find(
      (button) =>
        button.offsetParent !== null && button.textContent.trim() === 'Post',
    )
    if (!submit || submit.disabled)
      return api('/result', 'POST', {
        ok: false,
        kind: 'post',
        reason: 'Post button unavailable',
      })
    submit.click()
    await sleep(1000)
    return api('/result', 'POST', {
