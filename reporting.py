...
def build_report(root: Path, now: datetime | None = None) -> dict:
    ...
    report = {
        ...
        "correlation": {},
        "notes": [
            "Actions are counted only after the browser confirms them.",
            "Account views, received engagement, followers and website traffic come from snapshots.",
        ],
    }
    for label in WINDOWS:
        ...
        for row in events:
            ...
            if not captured or captured < cutoff or captured > now:
                continue
            mapped = ACTION_EVENTS.get(str(row.get("event")))
            if mapped:
    ...
    has_snapshots = bool(report["account_snapshots"] or report["website_analytics"])
...
def render_html(report: dict) -> str:
    ...
    windows = "".join(
        f"<section><h2>{label.title()}</h2><div class='grid'>{cards(data['actions'])}</div></section>"
        for label, data in report["windows"].items()
    )
    snapshots = {**report["account_snapshots"], **{f"GA4 — {k}": v for k, v in report["website_analytics"].items()}}
    snapshot_cards = cards({key: value.get("metrics", {}) for key, value in snapshots.items()})
...
.meta,.empty{{color:var(--muted)}}.phase{{display:inline-block;margin-top:12px;padding:6px 10px;border:1px solid #3d5675;border-radius:999px}}
...
<div class='phase'>State: {escape(report['phase'])}</div>{windows}<section><h2>Latest account and website snapshots</h2><div class='grid'>{snapshot_cards}</div></section>
...
def record_snapshot(root: Path, source: str, metrics: dict, captured_at: str | None = None) -> dict:
